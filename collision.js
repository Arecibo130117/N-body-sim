// collision.js
// Broadphase + Narrowphase + Impact Scalars + 보존량 기반 임펄스 반응(스핀 포함)
 ...
 * - 상대속도, 충돌각 성분(vN,vT), reduced mass, Eimp, Q
 */
export function computeImpactScalars(hit, pos, vel, mass) {
  const { i, j, n, dist, pen } = hit;
  ...
  return { i, j, n: n.clone(), dist, pen, relVel: rel, vRel, vN, vT, mu, Eimp, Q, contactPoint: cp };
}

/**
 * collisionImpulseResponse()
 * - 선형 운동량 보존 기반 + 마찰 임펄스로 스핀 변화 근사
 * - 용융(meltFrac) 높을수록 restitution 감소
 */
export function collisionImpulseResponse(impact, bodies, params) {
  ...
  if (vt > 1e-6) {
    ...
    const maxJt = muF * jn;
    jt = Math.min(maxJt, vt / (inv1 + inv2));
